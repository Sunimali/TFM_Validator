#Evaluate the model on eroded boundaries (NM2)
import pickle
import torch
...
import cv2
def add_gaussian_for_erode(traction, mask,std=0.001, mean=0, kernel_size=2):
...
for kernel_size in eroded_kernel_sizes:
    ...
    OUTPUT_BASE_PATH = '../img/validation/new/phase3/erode/'  # Base directory to save output images
    ...
    for i in range(x_trac_test.shape[0]):
        ...
        x_trac_test_dialated[i] = add_gaussian_for_erode(x_trac_test[i], x_trac_mask, kernel_size=kernel_size)
