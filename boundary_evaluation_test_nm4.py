...
import cv2
def add_gaussian_for_inside(traction,mask,std=0.01, mean=0, kernel_size=2):
    mask = mask.astype(np.uint8)
    
    # Dilate the mask to create a border area around high values
    kernel = np.ones((kernel_size, kernel_size), np.uint8)
   
    # shrink the mask to create a border area around high values
    dilated_mask = cv2.dilate(mask, np.ones((3,3), np.uint8), iterations=1)
    erode_mask = cv2.erode(dilated_mask, kernel, iterations=1)  # Expands the mask
    ...
    traction_noisy[:, erode_mask == 255] += noise[:, erode_mask == 255]  # Add noise to inside of  the border
   
    return traction_noisy

    
    return traction_noisy
def add_gaussian_for_erode_cell(mask,std=0.01, mean=0, kernel_size=2):
    ...
    return erode_mask
eroded_kernel_sizes = [3,4,5,6,7,8,9,10]

x_test_erode_per_kernel = []
y_test_erode_per_kernel = []

for kernel_size in eroded_kernel_sizes:
    ...
    OUTPUT_BASE_PATH = '../img/validation/new/phase2/nm4kxn01/'  # Base directory to save output images
    ...
    for i in range(y_test.shape[0]):
        y_test_dialated[i] = add_gaussian_for_erode_cell(y_test[i], kernel_size = kernel_size)
    ...
    for i in range(x_trac_test.shape[0]):
        ...
        #         traction_test_magnitude = np.sqrt(np.sum(x_tract[i,:,:,:]**2, axis=0))
        #         traction_test_magnitude = traction_test_magnitude / np.max(traction_test_magnitude)
        #         traction_test_magnitude = traction_test_magnitude * 255
        ...
        x_trac_test_dialated[i] = add_gaussian_for_inside(x_trac_test[i], x_trac_mask, kernel_size=kernel_size)
        x_test_eroded.append((x_img_test[i], x_trac_test_dialated[i]))

    x_test_erode_per_kernel.append(x_test_eroded)
    y_test_erode_per_kernel.append(y_test_dialated)
...
#save the eroded data
with open(DATA_PATH + 'x_test_nm4_per_kernel.pkl', 'wb') as f:
    pickle.dump(x_test_erode_per_kernel, f)
with open(DATA_PATH + 'y_test_nm4_per_kernel.pkl', 'wb') as f:
    pickle.dump(y_test_erode_per_kernel, f)
print("Eroded data saved successfully.")
