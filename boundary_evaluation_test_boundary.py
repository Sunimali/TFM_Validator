...
def add_gaussian_for_boundary(traction, mask,std=0.001, mean=0, kernel_size=2):
    ...
    dilated_mask = cv2.dilate(mask, np.ones((4,4), np.uint8), iterations=1)
...
def add_gaussian_for_erode_cell(mask,std=0.001, mean=0, kernel_size=2):
...
for kernel_size in eroded_kernel_sizes:
    ...
    OUTPUT_BASE_PATH = '../img/validation/new/phase3/boundary/'  # Base directory to save output images
    ...
    for i in range(x_trac_test.shape[0]):
        ...
        x_trac_test_dialated[i] = add_gaussian_for_boundary(x_trac_test[i], x_trac_mask, kernel_size=kernel_size)
...
with open(DATA_PATH + 'x_test_erode_per_kernel.pkl', 'wb') as f:
    pickle.dump(x_test_erode_per_kernel, f)
with open(DATA_PATH + 'y_test_erode_per_kernel.pkl', 'wb') as f:
